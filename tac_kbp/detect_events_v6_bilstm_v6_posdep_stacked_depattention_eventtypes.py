...
def extract_lbl_bio_stat(lbl_usage_stat):
    lbl_usage_stat_dict = dict(lbl_usage_stat)
    lbl_vocab = [xx[0][2:] if xx[0].startswith(u'B-') or xx[0].startswith(u'I-') else xx[0] for xx in
                 lbl_usage_stat]
    lbl_vocab = list(set(lbl_vocab))  # remove duplicates
    lbl_vocab.sort()

    lbl_usage_stat_bi = [[x,
                          0 if not "B-%s" % x in lbl_usage_stat_dict else lbl_usage_stat_dict["B-%s" % x],
                          0 if not "I-%s" % x in lbl_usage_stat_dict else lbl_usage_stat_dict["I-%s" % x],
                          0 if not x == "O" else lbl_usage_stat_dict[x]]
                         for x in lbl_vocab]

    for stat_rec in lbl_usage_stat_bi:
        if stat_rec[0] != "O":
            stat_rec[3] = stat_rec[1] + stat_rec[2]
    return lbl_usage_stat_bi

def log_field_label_statistics(data_meta, data_fieldname):
    lbl_usage_stat = TacPrepJsonCorpus.word_counts_from_data_meta(
        data=data_meta,
        data_fieldname=data_fieldname,
    )
    logging.info("%s Labels:%s" % (data_fieldname, lbl_usage_stat))

    lbl_usage_stat_bi = extract_lbl_bio_stat(lbl_usage_stat)
    logging.info("Nice formatting:")
    logging.info(tabulate(lbl_usage_stat_bi, headers=["Label", "B-", "I-", "All"]))

import tensorflow as tf

from tac_kbp.models import EventSequenceLabeler_BiLSTM_v6_posdep_stacked_depattention
from tac_kbp.utils.BatchHelpers import *


class EventLabelerAndClassifier_v6_BiLSTM_v6_posdep_stacked(object):
    def __init__(self,
                 classifier_name,
                 run_name,
                 model_dir,
        ...
        self._filter_sentences_with_labels = filter_sentences_with_labels
        pass
    ...
    def train(self,
              ...
              include_dep_tokens_layers,
              lstm_layers,
              learning_rate_fixed=True
              ):

        logging.info("train with class:[detect_events_v6_bilstm_v6_posdep_stacked_depattention]")
        ...
        data_y_fieldname = "labels_event"  # "labels_event", "labels_type_full"
        classification_type = "labels_event"  # type_full, labels_event
        ...
        word_usage_stat = TacPrepJsonCorpus.word_counts_from_jsonfiles(
            ...
            max_nr_sent=max_nr_sent,
            tokens_lowercase=lowercase_tokens
            )

        logging.info("Done in %s s" % (ti.time()-st))

        min_word_freq = 3  # 3 achieves the best score
        ...
        if classification_type == "type_full":
            ...
            lbl_usage_stat = TacPrepJsonCorpus.word_counts_from_jsonfiles(
                json_files=train_files + dev_files,
                data_fieldname=data_y_fieldname, # "labels_type_full",
                max_nr_sent=max_nr_sent
            )
            ...
            # filter labels
            if not self._filter_sentences_with_labels is None:
                lbl_vocab = []
                for xx in lbl_usage_stat:
                    lbl_candidate = xx[0]
                    skip_lbl = False
                    for lbl_start in self._filter_sentences_with_labels:
                        if xx[0].startswith(lbl_start):
                            skip_lbl = True

                    if skip_lbl:
                        continue

                    lbl_vocab.append(lbl_candidate)

            else:
                lbl_vocab = [xx[0] for xx in
                             lbl_usage_stat]
            ...
            if len(self._allowed_event_types) > 0:
                zero_label="O"
                other_label = "Event-Other"

                filtered_lbl_vocab = []
                for i in range(len(lbl_vocab)):
                    if canonicalize_string(lbl_vocab[i][2:] if lbl_vocab[i].startswith('B-') or lbl_vocab[i].startswith('I-') else lbl_vocab[i]) in self._allowed_event_types\
                            or lbl_vocab[i] == "O":
                        filtered_lbl_vocab.append(lbl_vocab[i])
                        mapping_labels[lbl_vocab[i]] = lbl_vocab[i]
                    else:
                        mapping_labels[lbl_vocab[i]] = other_label
            ...
            classes_dict = dict([(i+1, x) for i, x in enumerate(labels_lst)])
        else:
            labels_lst = [u'O', u'B-EVENT', u'I-EVENT']
            classes_dict = {1: u'O', 2: u'B-EVENT',  3: u'I-EVENT'}

        logging.info("Mappings:")
        for k,v in mapping_labels.iteritems():
            logging.info("%s - %s" % (v, k))

        logging.info("Mappings:")
        for k,v in classes_dict.iteritems():
            logging.info("%s - %s" % (v, k))
        # exit()
        ...
        train_seq, train_seq_meta = tac_corpus.read_sequence_list_tac_json(train_files,
                                                                           ...
                                                                           lowercase_data_x=lowercase_tokens,
                                                                           filter_sentences_with_labels=self._filter_sentences_with_labels
                                                                           )

        # Retrieve Event Types
        logging.info("Retrieve EVENT TYPE statistics..")
        log_field_label_statistics(train_seq_meta, "labels_type_full")

        # Retrieve Event Types
        logging.info("Retrieve EVENT REALIS statistics..")
        log_field_label_statistics(train_seq_meta, "labels_realis")
        ...
        cnt_cleared = clear_labels_for_eventtypes_notallowed(train_seq, train_seq_meta,
                                                             allowed_types=self._allowed_event_types, zero_y_label=1,
                                                             zero_label_str="O")
        ...
        # Retrieve dependncy tokens
        logging.info("extracting dependency features for train...")
        train_dep_tokens_left, train_dep_tokens_right, train_dep_tokens_left_insent_ids, train_dep_tokens_right_insent_ids = Tac2016_EventNuggets_DataUtilities\
            .get_left_right_data_token_idx_for_dep_tokens(data_meta=train_seq_meta,
                                                           items_input_seq=train_seq)

        logging.info("train_dep_tokens_left[0]:%s" % train_dep_tokens_left[0])
        logging.info("train_dep_tokens_right[0]:%s" % train_dep_tokens_right[0])
        # logging.info("train_dep_tokens_full[0]:%s" % train_dep_tokens_full[0])
        ...
        if eval_dev and len(dev_files) > 0:
            ...
            # Retrieve Event Types
            logging.info("Retrieve EVENT TYPE statistics..")
            log_field_label_statistics(dev_seq_meta, "labels_type_full")

            # Retrieve Event Types
            logging.info("Retrieve EVENT REALIS statistics..")
            log_field_label_statistics(dev_seq_meta, "labels_realis")
            ...
            # Retrieve dependncy tokens
            logging.info("extracting dependency features for dev")
            dev_dep_tokens_left, dev_dep_tokens_right, dev_dep_tokens_left_insent_ids, dev_dep_tokens_right_insent_ids = Tac2016_EventNuggets_DataUtilities\
                .get_left_right_data_token_idx_for_dep_tokens(data_meta=dev_seq_meta,
                                                           items_input_seq=dev_seq)

            logging.info("dev_dep_tokens_left[0]:%s" % dev_dep_tokens_left[0])
            logging.info("dev_dep_tokens_right[0]:%s" % dev_dep_tokens_right[0])
            # logging.info("dev_dep_tokens_full[0]:%s" % dev_dep_tokens_full[0])
        ...
        self.train_sequencelabeler_and_save_model(train_seq=train_seq,
                                  ...
                                  train_dep_tokens_left=train_dep_tokens_left,
                                  train_dep_tokens_right=train_dep_tokens_right,
                                  train_dep_tokens_left_insent_ids=train_dep_tokens_left_insent_ids,
                                  train_dep_tokens_right_insent_ids=train_dep_tokens_right_insent_ids,
                                  dev_dep_tokens_left=dev_dep_tokens_left,
                                  dev_dep_tokens_right=dev_dep_tokens_right,
                                  dev_dep_tokens_left_insent_ids=dev_dep_tokens_left_insent_ids,
                                  dev_dep_tokens_right_insent_ids=dev_dep_tokens_right_insent_ids,
                                   eval_dev=eval_dev,
                                  ...
                                  include_dep_tokens_layers=include_dep_tokens_layers,
    ...
    def train_sequencelabeler_and_save_model(self,
                             ...
                             dev_deps_left,
                             dev_deps_right,
                                 train_dep_tokens_left,
                                 train_dep_tokens_right,
                                 train_dep_tokens_left_insent_ids,
                                 train_dep_tokens_right_insent_ids,
                                 dev_dep_tokens_left,
                                 dev_dep_tokens_right,
                                 dev_dep_tokens_left_insent_ids,
                                 dev_dep_tokens_right_insent_ids,
                              eval_dev,
                              n_classes,
                              classes_dict,
                              embeddings,
        ...
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                seq_model = EventSequenceLabeler_BiLSTM_v6_posdep_stacked_depattention(
                                       n_classes=n_classes,
                                       embeddings=embeddings,
                                       embeddings_size=embeddings_size,
                                       embeddings_number=embeddings_number,
                ...
                capped_gvs = [(tf.clip_by_value(tf_helpers.tf_nan_to_zeros_float32(grad), -1., 1.) if grad is not None else grad, var) for grad, var in
                                   gvs]  # cap to prevent NaNs
                # capped_gvs = [(tf.clip_by_value(tf_helpers.tf_nan_to_zeros_float32(grad), -1., 1.), var) for grad, var in
                #                   gvs]  # cap to prevent NaNs
                ...
                with tf.name_scope("accuracy"):
                    ...
                    mask = tf.sign(tf.cast(seq_model.input_y_flat, dtype=tf.float32))
                    ...
                    input_seq_len_float = tf.cast(seq_model.input_seq_len, dtype=tf.float32)
                ...
                def train_step(sess, seq_model,
                               ...
                               x_batch_deps_mask,
                               x_batch_dep_tokens,
                               x_batch_dep_tokens_len,
                               x_batch_dep_tokens_mask):
                    ...
                    feed_dict = {
                        ...
                        seq_model.input_x_dep_tokens: x_batch_dep_tokens,
                        seq_model.input_x_dep_tokens_len: x_batch_dep_tokens_len,
                        seq_model.input_x_dep_tokens_mask: x_batch_dep_tokens_mask,
                ...
                for epoch in range(0, epochs_cnt):
                    ...
                    for i in range(0, train_batches_cnt):
                        ...
                        if len(batch_cache_train)>i:
                            ...
                            batch_data_dep_tokens_left = curr_batch_cache["batch_data_dep_tokens_left"]
                            batch_data_dep_tokens_left_seqlens = curr_batch_cache["batch_data_dep_tokens_left_seqlens"]
                            batch_data_dep_tokens_left_masks = curr_batch_cache["batch_data_dep_tokens_left_masks"]
                            ...
                            # deps tokens
                            batch_data_dep_tokens_left = train_dep_tokens_left[batch_from: batch_to]
                            batch_data_dep_tokens_left, batch_data_dep_tokens_left_seqlens, batch_data_dep_tokens_left_masks = pad_and_get_mask(
                                batch_data_dep_tokens_left, pad_value=0)
                            batch_data_dep_tokens_left_seqlens = [pad(xx, pad_value=0, to_size=max_batch_sent_len) for xx in batch_data_dep_tokens_left_seqlens]
                            ...
                            curr_batch_cache = {"batch_data_padded_x":batch_data_padded_x,
                                                ...
                                                "batch_data_dep_tokens_left": batch_data_dep_tokens_left,
                                                "batch_data_dep_tokens_left_seqlens": batch_data_dep_tokens_left_seqlens,
                                                "batch_data_dep_tokens_left_masks": batch_data_dep_tokens_left_masks
                                                }
                        ...
                        cost, acc = train_step(sess, seq_model,
                                               ...
                                               x_batch_dep_tokens=batch_data_dep_tokens_left,
                                               x_batch_dep_tokens_len=batch_data_dep_tokens_left_seqlens,
                                               x_batch_dep_tokens_mask=batch_data_dep_tokens_left_masks
                                               )
                    ...
                    if eval_dev and epoch >= 0 and (epoch+1) % eval_every == 0:
                        ...
                        for i in range(0, dev_batches_cnt):
                            ...
                            if len(batch_cache_dev) > i:
                                ...
                                batch_data_dep_tokens_left = curr_batch_cache["batch_data_dep_tokens_left"]
                                batch_data_dep_tokens_left_seqlens = curr_batch_cache[
                                    "batch_data_dep_tokens_left_seqlens"]
                                batch_data_dep_tokens_left_masks = curr_batch_cache["batch_data_dep_tokens_left_masks"]
                                # logging.info("Loaded batch %s from cache" % i)
                                # print batch_data_padded_x
                            else:
                                ...
                                # deps tokens
                                batch_data_dep_tokens_left = dev_dep_tokens_left[batch_from: batch_to]
                                batch_data_dep_tokens_left, batch_data_dep_tokens_left_seqlens, batch_data_dep_tokens_left_masks = pad_and_get_mask(
                                    batch_data_dep_tokens_left, pad_value=0)
                                batch_data_dep_tokens_left_seqlens = [pad(xx, pad_value=0, to_size=max_batch_sent_len)
                                                                      for xx in batch_data_dep_tokens_left_seqlens]
                        ...
                        if len(classes_dict) > 4:
                            ...
                            # eval - even/non-event
                            logging.info("Confusion matrix - Even/non-event :")
                            input_y_all_mapped = [1 if y not in classes_dict or classes_dict[y] == "O" else 2 for y in input_y_all]
                            pred_y_all_mapped = [1 if y not in classes_dict or classes_dict[y] == "O" else 2 for y in pred_y_all]

                            conf_matrix = confusion_matrix(input_y_all_mapped, pred_y_all_mapped, labels=[1, 2])
                            logging.info("\n"+str(conf_matrix))
                            logging.info("Results by class:")
                            # print_acc_from_conf_matrix(conf_matrix, classes_dict)
                            p_r_f1_acc_by_class_dict = get_prec_rec_fscore_acc_from_conf_matrix(conf_matrix, {1:"O", 2:"B-EVENT"})
                            logging.info("label: (prec, recall, f-score, accuracy)")
                            for k,v in p_r_f1_acc_by_class_dict.iteritems():
                                logging.info("%s:%s" % (k, str(v)))
                        else:
    ...
    def eval(self,
        ...
        logging.info("eval with class:[detect_events_v6_bilstm_v6_posdep_stacked_depattention]")
        # Load data settings
        lowercase_tokens = self._lowercase_tokens
        max_sent_len = 1000
        ...
        # Retrieve Event Types
        logging.info("Retrieve EVENT TYPE statistics..")
        log_field_label_statistics(test_seq_meta, "labels_type_full")

        # Retrieve Event Types
        logging.info("Retrieve EVENT REALIS statistics..")
        log_field_label_statistics(test_seq_meta, "labels_realis")

        logging.info("Test export!!!")
        # Extract events
        ...
        Tac2016_EventNuggets_DataUtilities.save_data_to_json_file(test_seq_meta, output_json_file+".gold.json.txt")
        logging.info("Processed json saved to %s" % output_json_file+".gold.json.txt")

        # Writing submission file
        system_name = "aiphes_hd_t16"
        output_file = "%s/%s_submission_output.tbf.gold.tbf" % (self._output_dir, self._run_name)
        Tac2016_EventNuggets_DataUtilities.save_to_output_tbf(output_file, event_nuggets_by_docs, system_name)
        logging.info("Gold Submission file saved to %s" % output_file)
        if filter_labels:
            logging.info("Clearing labels...to match allowed event tyeps")
        ...
        # Retrieve dependncy tokens
        logging.info("extracting dependency features for test")
        test_dep_tokens_left, test_dep_tokens_right, test_dep_tokens_left_insent_ids, test_dep_tokens_right_insent_ids = Tac2016_EventNuggets_DataUtilities\
            .get_left_right_data_token_idx_for_dep_tokens(data_meta=test_seq_meta,
                                                           items_input_seq=test_seq)

        logging.info("test_dep_tokens_left[0]:%s" % test_dep_tokens_left[0])
        logging.info("test_dep_tokens_right[0]:%s" % test_dep_tokens_right[0])
        # logging.info("test_dep_tokens_full[0]:%s" % test_dep_tokens_full[0])
...
if __name__ == "__main__":
    ...
    # filter_sentences_with_labels vim = None # ="I-"
    filter_sentences_with_labels = ["I-"]

    lowercase = False
    lowercase = CommonUtilities.get_param_value_bool("lowercase", sys.argv, lowercase)
    logging.info('lowercase:{0}'.format(lowercase))
    ...
    classifier_name = "event_labeler_BiLSTM_v6_posdeps_depattention"
    magic_box = EventLabelerAndClassifier_v6_BiLSTM_v6_posdep_stacked(
                    ...
                    filter_sentences_with_labels=filter_sentences_with_labels
                )
    ...
    if cmd == "train":
        ...
        # Dep Tokens - Include dep token embeddings on input layer
        incl_deptokenemb_in = False
        incl_deptokenemb_in = CommonUtilities.get_param_value_bool("incl_deptokenemb_in", sys.argv,
                                                              default=incl_deptokenemb_in)
        logging.info('incl_deptokenemb_in:%s' % incl_deptokenemb_in)

        # DEP Tokens - Include dep embeddings on output layer
        incl_deptokenemb_out = False
        incl_deptokenemb_out = CommonUtilities.get_param_value_bool("incl_deptokenemb_out", sys.argv,
                                                               default=incl_deptokenemb_out)
        logging.info('incl_deptokenemb_out:%s' % incl_deptokenemb_out)
        ...
        include_dep_tokens_layers = {'input_layer': incl_deptokenemb_in, 'output_layer': incl_deptokenemb_out}
