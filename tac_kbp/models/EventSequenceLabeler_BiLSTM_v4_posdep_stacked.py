...
import logging
class EventSequenceLabeler_BiLSTM_v4_posdep_stacked(object):
    def __init__(self,
                n_classes,
                embeddings,
                embeddings_size,
        ...
        output_layer = None
        output_layer_size = 0
        bilstm_layers_cnt = bilstm_layers
        ...
        for i in range(0, bilstm_layers_cnt):
            ...
            reverse_bw=False
            logging.info('Reverse BW output:%s' % reverse_bw)
            if reverse_bw:
                ...
                output_layer = tf.concat(2, outputs, name="output_layer")
